...
public class JPADAOImpl<K, E> implements JPADAO<E, K> {
	...
	public List<E> find(final int startFrom, final int maxResults) {
		 ...
		 * 
		 * public Object doInJpa(EntityManager em) throws PersistenceException { try{
		 * Query q = em.createQuery("SELECT h FROM " + entityClass.getName() + " h");
		 * q.setMaxResults(maxResults); q.setFirstResult(startFrom); return
		 * q.getResultList(); } catch(PersistenceException e){ e.printStackTrace();
		 * throw e; } }
		 * 
		 * }));
		 */

		Query q = getEntityManager().createQuery("SELECT h FROM " + entityClass.getName() + " h ORDER BY h.id DESC");
